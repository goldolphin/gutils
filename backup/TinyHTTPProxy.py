...
2012-06-01 - Modified by goldolphin
             * Added bind hostname option: -n
...
def usage (msg=None):
    if msg: print msg
    print sys.argv[0], "[-n hostname] [-p port] [-l logfile] [-dh] [allowed_client_name ...]]"
    print
    print "   -n       - Hostname to bind to"
...
def main ():
    ...
    try: opts, args = getopt.getopt (sys.argv[1:], "l:dhn:p:", [])
    except getopt.GetoptError, e:
        usage (str (e))
        return 1

    for opt, value in opts:
        if opt == "-n": local_hostname = value
